define([
  ...
  "handlebars",
   ...
   "appconfig",
    "logger"
],function($, _, Backbone, Handlebars, dialog, Router, config, Logger){
    ...
    init : function() {
       ...
       //INSERT ROUTER??
        ...
        Backbone.View.prototype.close = function(){ //remove zombie views
            if(this.model)
                Logger.debug("removed view w/ model:", this.model.toJSON());
            if(this.collection)
                Logger.debug("removed view w/ collection:", this.collection.toJSON());
            this.remove();
            this.unbind();
            if (this.onClose){
                this.onClose();
